...
module.exports = function AbstractS3PutTriggerCommon() {
  ...
  AbstractS3PutTriggerCommon.prototype.AbstractBaseCommon.beforeMainExecute = function (
    ...
    try {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var LineStream = base.RequireObjects.ByLine.LineStream;
        var lineStreamObj = new LineStream();

        // 読み込んだデータ行を一時的に貯め込む変数
        var buffArray = [];

        // resolveで次へ渡す変数
        var rtnArray = [];

        var batchPutSize = base.getBatchPutSize();
        var readLineCount = 0;

        s3stream.pipe(lineStreamObj).on("data", function (line) {
          readLineCount++;
          buffArray.push(line);

          if (buffArray.length > batchPutSize) {
            var targetArray = buffArray.splice(0, batchPutSize);
            rtnArray.push(targetArray);
          }
        });

        s3stream.on("end", function (line) {
          if (buffArray.length > 0) {
            var targetArray = buffArray.splice(0, batchPutSize);
            rtnArray.push(targetArray);
          }

          base.writeLogInfo("AbstractS3PutTriggerCommon# S3 Stream Read End");
          var result = { workerArgs: rtnArray };
          resolve(result);
        });
  ...
  */
  ...
  @param args 各処理の結果を格納した配列
  */
  AbstractS3PutTriggerCommon.prototype.AbstractBaseCommon.afterUnProcessedItemCheck = function (
    args
  ) {
    var base = AbstractS3PutTriggerCommon.prototype.AbstractBaseCommon;
    try {
      base.writeLogTrace(
        "AbstractS3PutTriggerCommon# afterUnProcessedItemCheck : start"
      );

      var unprocessedItems = base.getLastIndexObject(args);

      return new Promise(function (resolve, reject) {
        var unprocessedCheckFlag = new Boolean(false);
        if (unprocessedItems && unprocessedItems.length > 0) {
          for (var i = 0; i < unprocessedItems.length; i++) {
            var unprocessedItem = unprocessedItems[i];
            if (Object.keys(unprocessedItem).length > 0) {
              unprocessedCheckFlag = new Boolean(true);
              break;
            }
          }
        }

        if (unprocessedCheckFlag == true) {
          base.writeLogWarn("AbstractS3PutTriggerCommon# Fail DynamoDB Put");
          reject("AbstractS3PutTriggerCommon# Fail DynamoDB Put");
        } else {
          base.writeLogInfo(
            "AbstractS3PutTriggerCommon# All Records Success Finish"
          );
          resolve("AbstractS3PutTriggerCommon# All Records Success Finish");
        }
      });
    } catch (err) {
      base.printStackTrace(err);
      ...
      base.writeLogTrace(
        "AbstractS3PutTriggerCommon# afterUnProcessedItemCheck : end"
      );
    }
  }.bind(AbstractS3PutTriggerCommon.prototype.AbstractBaseCommon);
  ...
  */
  AbstractS3PutTriggerCommon.prototype.AbstractBaseCommon.getSubWorkerClazzFunc = function () {
    ...
    try {
      ...
      return require("./AbstractWorkerDynamoDbBatchPutCommon.js");
    } catch (err) {
      base.printStackTrace(err);
      throw err;
    } finally {
