...
module.exports = function SampleS3PutTriggerModule() {
  ...
  // DynamoDBのAPIのbatchPutの上限は24
  var BATCH_PUT_SIZE = 5;
  if (process && process.env && process.env.BatchPutSize) {
    BATCH_PUT_SIZE = process.env.BatchPutSize;
  }
  ...
  SampleS3PutTriggerModule.prototype.AbstractBaseCommon.getBatchPutSize = function () {
    var base = SampleS3PutTriggerModule.prototype.AbstractBaseCommon;
    try {
      base.writeLogTrace("SampleS3PutTriggerModule# getBatchPutSize : start");
      return BATCH_PUT_SIZE;
    } catch (err) {
      base.printStackTrace(err);
      throw err;
    } finally {
      base.writeLogTrace("SampleS3PutTriggerModule# getBatchPutSize : end");
    }
  ...
  */
  SampleS3PutTriggerModule.prototype.AbstractBaseCommon.getSubWorkerClazzFunc = function () {
    ...
    try {
      ...
      return require("./SampleWorkerDynamoDbBatchPutModule.js");
    } catch (err) {
      base.printStackTrace(err);
      throw err;
    } finally {
